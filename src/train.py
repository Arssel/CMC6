...
def train(model, device="cuda", batch_size=256, epochs=100, T=40, lr=1e-4, problem_size=20, decay=0.001,
          ...
          return_distances=False, distance_step=5, r_val=None, r_test=None):
    ...
    for e in tqdm(range(epochs), leave=False):
        ...
        for step in range(T):
            ...
            if r_val is not None:
                features, distances, mask = env.reset(r=r_val)
                features[0] = features[0].to(device)
                features[1] = features[1].to(device)
                flag_done = False
                t = 0
                precomputed = None
                while not flag_done:
                    v, _, precomputed = model(features, mask, t, precomputed, True)
                    v = v.to('cpu')
                    features, mask, flag_done = env.step(v)
                    features[1] = features[1].to(device)
                    t += 1
                mf = (env.vehicle[:, :, -1].sum(dim=1).to('cpu') +
                      check_missing_vertexes_jampr(env.tour_plan, problem_size) * penalty_num_vertexes).mean()
                reward_val_list.append(mf.detach().item())

            if r_test is not None:
                features, distances, mask = env.reset(r=r_test)
                features[0] = features[0].to(device)
                features[1] = features[1].to(device)
                flag_done = False
                t = 0
                ...
                while not flag_done:
                    ...
                    features[1] = features[1].to(device)
                    t += 1
                mf_test = (env.vehicle[:, :, -1].sum(dim=1).to('cpu') +
                      check_missing_vertexes_jampr(env.tour_plan, problem_size) * penalty_num_vertexes).mean()
                reward_test_list.append(mf_test.detach().item())
            ...
            if mf < best_reward:
                ...
                if save_inbetween:
                    ...
                    f.close()
                    if r_val is not None:
                        f = open(output + file_name + '_reward_val_on_iteration.pkl', 'wb')
                        pickle.dump(reward_val_list, f)
                        f.close()
                    if r_test is not None:
                        f = open(output + file_name + '_reward_test_on_iteration.pkl', 'wb')
                        pickle.dump(reward_test_list, f)
                        f.close()
                #print(best_reward)
            optimizer.zero_grad()
    ...
    #print(reward_list)
    if r_val is not None and r_test is not None:
        return best_weights, model.state_dict(), file_name, loss_list, reward_list, reward_val_list, reward_test_list
    elif r_val is not None:
        return best_weights, model.state_dict(), file_name, loss_list, reward_list, reward_val_list
    elif r_test is not None:
        return best_weights, model.state_dict(), file_name, loss_list, reward_list, reward_test_list
